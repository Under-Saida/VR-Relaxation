...
public class EEGdata : MonoBehaviour
{
    ...
    void OnUpdateMindwaveData(MindwaveDataModel _Data)
    {
        ...
        attention1 = m_MindwaveData.eSense.attention;
        StreamWriter attention = new StreamWriter("../data/EEGdata/attentionData.csv", true, Encoding.GetEncoding("Shift_JIS"));
        attention.WriteLine(attention1);
        attention.Close();

        meditation1 = m_MindwaveData.eSense.meditation;
        StreamWriter meditation = new StreamWriter("../data/EEGdata/meditationData.csv", true, Encoding.GetEncoding("Shift_JIS"));
        meditation.WriteLine(meditation1);
        meditation.Close();

        Delta = m_MindwaveData.eegPower.delta;
        StreamWriter delta = new StreamWriter("../data/EEGdata/deltaData.csv", true, Encoding.GetEncoding("Shift_JIS"));
        delta.WriteLine(Delta);
        delta.Close();

        Theta = m_MindwaveData.eegPower.theta;
        StreamWriter theta = new StreamWriter("../data/EEGdata/thetaData.csv", true, Encoding.GetEncoding("Shift_JIS"));
        theta.WriteLine(Theta);
        theta.Close();

        LowAlpha = m_MindwaveData.eegPower.lowAlpha;
        StreamWriter la = new StreamWriter("../data/EEGdata/lowalphaData.csv", true, Encoding.GetEncoding("Shift_JIS"));
        la.WriteLine(LowAlpha);
        la.Close();

        HighAlpha = m_MindwaveData.eegPower.highAlpha;
        StreamWriter ha = new StreamWriter("../data/EEGdata/highalphaData.csv", true, Encoding.GetEncoding("Shift_JIS"));
        ha.WriteLine(HighAlpha);
        ha.Close();

        LowBeta = m_MindwaveData.eegPower.lowBeta;
        StreamWriter lb = new StreamWriter("../data/EEGdata/lowbetaData.csv", true, Encoding.GetEncoding("Shift_JIS"));
        lb.WriteLine(LowBeta);
        lb.Close();

        HighBeta = m_MindwaveData.eegPower.highBeta;
        StreamWriter hb = new StreamWriter("../data/EEGdata/highbetaData.csv", true, Encoding.GetEncoding("Shift_JIS"));
        hb.WriteLine(HighBeta);
        hb.Close();

        LowGamma = m_MindwaveData.eegPower.lowGamma;
        StreamWriter lg = new StreamWriter("../data/EEGdata/lowgammaData.csv", true, Encoding.GetEncoding("Shift_JIS"));
        lg.WriteLine(LowGamma);
        lg.Close();

        HighGamma = m_MindwaveData.eegPower.highGamma;
        StreamWriter hg = new StreamWriter("../data/EEGdata/highgammaData.csv", true, Encoding.GetEncoding("Shift_JIS"));
