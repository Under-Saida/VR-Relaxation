...
public class EEGdata : MonoBehaviour
{
    ...
    void OnUpdateMindwaveData(MindwaveDataModel _Data)
    {
        ...
        attention1 = m_MindwaveData.eSense.attention;
        StreamWriter attention = new StreamWriter("../EEGdata/attentionData.txt", true, Encoding.GetEncoding("Shift_JIS"));
        attention.WriteLine(attention1);
        attention.Close();

        meditation1 = m_MindwaveData.eSense.meditation;
        StreamWriter meditation = new StreamWriter("../EEGdata/meditationData.txt", true, Encoding.GetEncoding("Shift_JIS"));
        meditation.WriteLine(meditation);
        meditation.Close();

        Delta = m_MindwaveData.eegPower.delta;
        StreamWriter delta = new StreamWriter("../EEGdata/deltaData.txt", true, Encoding.GetEncoding("Shift_JIS"));
        delta.WriteLine(Delta);
        delta.Close();

        Theta = m_MindwaveData.eegPower.theta;
        StreamWriter theta = new StreamWriter("../EEGdata/thetaData.txt", true, Encoding.GetEncoding("Shift_JIS"));
        theta.WriteLine(Theta);
        theta.Close();

        LowAlpha = m_MindwaveData.eegPower.lowAlpha;
        StreamWriter la = new StreamWriter("../EEGdata/lowalphaData.txt", true, Encoding.GetEncoding("Shift_JIS"));
        la.WriteLine(LowAlpha);
        la.Close();

        HighAlpha = m_MindwaveData.eegPower.highAlpha;
        StreamWriter ha = new StreamWriter("../EEGdata/highalphaData.txt", true, Encoding.GetEncoding("Shift_JIS"));
        ha.WriteLine(HighAlpha);
        ha.Close();

        LowBeta = m_MindwaveData.eegPower.lowBeta;
        StreamWriter lb = new StreamWriter("../EEGdata/lowbetaData.txt", true, Encoding.GetEncoding("Shift_JIS"));
        lb.WriteLine(LowBeta);
        lb.Close();

        HighBeta = m_MindwaveData.eegPower.highBeta;
        StreamWriter hb = new StreamWriter("../EEGdata/highbetaData.txt", true, Encoding.GetEncoding("Shift_JIS"));
        hb.WriteLine(HighBeta);
        hb.Close();

        LowGamma = m_MindwaveData.eegPower.lowGamma;
        StreamWriter lg = new StreamWriter("../EEGdata/lowgammaData.txt", true, Encoding.GetEncoding("Shift_JIS"));
        lg.WriteLine(LowGamma);
        lg.Close();

        HighGamma = m_MindwaveData.eegPower.highGamma;
        StreamWriter hg = new StreamWriter("../EEGdata/highgammaData.txt", true, Encoding.GetEncoding("Shift_JIS"));
